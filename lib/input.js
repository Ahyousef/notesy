...
class Input {
    ...
    constructor() {
        ...
        this.category = this.validateCategory(category,Object.values(arg)[2]);
        if (this.command == false) {
            ...
            console.log('For category use --category SOMECATEGORY');
            mongoose.disconnect();
        }
        else if (category == 'category' && this.category == false){
            console.log('For category use --category SOMECATEGORY');
        }
        else { 
            return {
    ...
    validateCategory(arg,argVal) {
        if (arg == "category" && argVal !== undefined && argVal !== '' && argVal.length > 0) {
            return true
        }
        else { return false }
    }
